...
function rotate() {
	...
	// refresh the server "date" and time
        ...
        var mcyear = (serverClock/24/1000+0.5)/365.25+1;
	$('#date').text(d.toUTCString().substr(5,7) + Math.floor(mcyear));
	$('#time').text(d.toUTCString().substr(17,5));
...
function sync() {
 	$.getJSON('/mc_data/time/getServerTime.php',
		function(data) {
